...
| `fantasy.witch` | Partial | Server/CPU Poison Cloud creation, source-death cleanup and owner-turn damage triggering are wired, with CPU generation included. Local parity and regression coverage remain. |
| `fantasy.druid` | Partial | Server/CPU Bramble creation and CPU generation are wired; shared entry/self-damage/no-landing rules now trigger in both server and CPU movement. Local parity and regression coverage remain. |
...
| `fantasy.dragon` | Partial | Forward-Line attack is wired/tested and Fire immunity is shared; server/CPU Fire triggering respects that immunity. Local Fire-trigger integration remains before verification. |
| `fantasy.gargoyle` | Verified | No-Man's-Land placement is enforced locally, by the server, and in CPU simulation. |
| `fantasy.phoenix` | Partial | Shared terrain/Fire immunity plus Fire trigger rules are defined; server/CPU Fire creation and CPU generation are wired, and Fire triggers in server/CPU movement. Local parity and regression coverage remain. |
