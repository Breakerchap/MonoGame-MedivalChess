...
| `fantasy.witch` | Partial | Poison Cloud creation, online mapping, owner-turn damage and source-death cleanup are now wired across local/server/CPU, with CPU generation included. Regression coverage remains before verification. |
| `fantasy.druid` | Partial | Bramble creation/online mapping, entry damage, Bramble self-damage and no-landing are wired across local/server/CPU, with CPU generation included. Regression coverage remains before verification. |
...
| `fantasy.dragon` | Partial | Forward-Line attack is wired/tested and shared Fire immunity applies through local/server/CPU Fire triggering. Dedicated runtime Fire-immunity coverage remains before verification. |
| `fantasy.gargoyle` | Verified | No-Man's-Land placement is enforced locally, by the server, and in CPU simulation. |
| `fantasy.phoenix` | Partial | Terrain/Fire immunity, Fire creation/online mapping and Fire triggering are wired across local/server/CPU; its active ability remains usable after a normal attack and CPU generation is included. Regression coverage remains. |
