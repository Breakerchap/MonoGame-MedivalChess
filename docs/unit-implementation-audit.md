...
| `fantasy.witch` | Partial | Server and CPU Poison Cloud creation are wired; CPU source-death cleanup is wired and shared owner-turn damage rules exist. CPU turn triggering, local parity, and regression coverage remain. |
| `fantasy.druid` | Partial | Server and CPU Bramble creation are wired; shared entry/self-damage and no-landing rules exist, and CPU movement triggering is connected. Local parity and regression coverage remain. |
...
| `fantasy.dragon` | Partial | Forward-Line multi-target attack is wired/tested and shared Fire immunity is defined; CPU Fire triggering now respects that immunity. Server/local Fire-trigger integration remains. |
| `fantasy.gargoyle` | Verified | No-Man's-Land placement is enforced locally, by the server, and in CPU simulation. |
| `fantasy.phoenix` | Partial | Shared terrain/Fire immunity plus Fire trigger rules are defined; server and CPU Fire creation are wired and CPU movement triggering is connected. Local parity and regression coverage remain. |
