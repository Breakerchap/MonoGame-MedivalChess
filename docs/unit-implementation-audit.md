...
| `fantasy.witch` | Partial | Server/CPU Poison Cloud creation plus CPU source-death cleanup and owner-turn damage triggering are wired; CPU generation is included. Local parity and regression coverage remain. |
| `fantasy.druid` | Partial | Server/CPU Bramble creation and CPU action generation are wired; shared entry/self-damage, no-landing, and CPU movement triggering are connected. Local parity and regression coverage remain. |
...
| `fantasy.phoenix` | Partial | Shared terrain/Fire immunity plus Fire trigger rules are defined; server/CPU Fire creation, CPU generation and CPU movement triggering are wired. Local parity and regression coverage remain. |
