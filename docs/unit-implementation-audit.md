...
| `fantasy.witch` | Partial | Poison Cloud damage/source cleanup now trigger in local/server/CPU, while server/CPU creation and CPU generation are wired. Local ability creation/online mapping and regression coverage remain. |
| `fantasy.druid` | Partial | Bramble entry/self-damage/no-landing now trigger in local/server/CPU; server/CPU creation and CPU generation are wired. Local ability creation/online mapping and regression coverage remain. |
...
| `fantasy.dragon` | Partial | Forward-Line attack is wired/tested and shared Fire immunity now applies through local/server/CPU Fire triggering. A dedicated Fire-immunity runtime regression remains before verification. |
| `fantasy.gargoyle` | Verified | No-Man's-Land placement is enforced locally, by the server, and in CPU simulation. |
| `fantasy.phoenix` | Partial | Terrain/Fire immunity and Fire triggering now work in local/server/CPU; server/CPU creation and CPU generation are wired. Local ability creation/online mapping and regression coverage remain. |
