...
public static partial class CpuGameRules
{
  private static int ApplyCpuChessKingDeathRule(CpuMutableGameState state, NetworkPiece target, int damage)
  {
    ...
    return ChessAbilityRules.CanChessKingDie(IsCpuChessKingCheckmated(state, target))
      ? damage
  ...
  private static bool CanCpuChessKingOccupy(
  ...
  )
  {
    if (!BoardRules.FootprintFitsBoard(state.Source.Board, destination.x, destination.y, kingRule.Width, kingRule.Height) ||
        OccupiedSquares(kingRule, destination).Any(square =>
          state.Source.Terrain.IsLake(square) || state.Barricades.ContainsKey(square)))
    {
      return false;
    }

    NetworkPiece? occupant = state.Pieces.FirstOrDefault(other =>
  ...
  private static bool IsCpuChessKingSquareThreatened(
    CpuMutableGameState state,
    NetworkPiece king,
    (int x, int y) square
  )
  {
    NetworkPiece? capturedAtDestination = square == (king.X, king.Y)
      ? null
      : state.Pieces.FirstOrDefault(other => other.Id != king.Id && other.AttachedToId is null &&
    ...
    foreach (NetworkPiece attacker in state.Pieces.Where(piece =>
      ...
      piece.Team != king.Team && piece.Team != NetworkTeam.Neutral))
    {
      ...
      if (ChessAbilityRules.IsLandingCaptureUnit(attacker.Type))
      {
        if (ChessAbilityRules.HasClearLandingCapturePath(
          ...
          square,
          intermediate => CpuChessThreatBlocker(state, intermediate, attacker, king, capturedAtDestination)))
        {
          return true;
        }
        continue;
      ...
      if (LineOfSightRules.HasClearAttackPath(
        ...
        square,
        state.Source.Terrain.IsForest,
        state.Barricades.ContainsKey,
        intermediate => state.Pieces.Any(other =>
  ...
  private static bool CpuChessThreatBlocker(
    CpuMutableGameState state,
    (int x, int y) square,
