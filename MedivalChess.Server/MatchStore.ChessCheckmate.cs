...
public sealed partial class MatchStore
{
  ...
  private static bool CanServerChessKingOccupy(Match match, NetworkPiece king, UnitRule kingRule, (int x, int y) destination)
  {
    if (!NetworkPieceRules.FootprintFitsBoard(match.Configuration, destination.x, destination.y, kingRule.Width, kingRule.Height) ||
        OccupiedSquares(kingRule, destination).Any(square => match.Terrain.IsLake(square) || match.Barricades.ContainsKey(square))) return false;
  ...
  private static bool IsServerChessKingSquareThreatened(Match match, NetworkPiece king, (int x, int y) square)
  {
    NetworkPiece? capturedAtDestination = square == (king.X, king.Y) ? null : match.Pieces.FirstOrDefault(other =>
      other.Id != king.Id && other.AttachedToId is null && other.Type != nameof(PieceType.Farm) && other.Team != king.Team &&
      UnitRules.TryGet(other.Type, out UnitRule otherRule) && OccupiedSquares(otherRule, (other.X, other.Y)).Contains(square));
      ...
      piece.Team != king.Team && piece.Team != NetworkTeam.Neutral))
    {
      ...
      if (ChessAbilityRules.IsLandingCaptureUnit(attacker.Type))
      {
        if (ChessAbilityRules.HasClearLandingCapturePath(attackerRule, (attacker.X, attacker.Y), square,
          intermediate => ServerChessThreatBlocker(match, intermediate, attacker, king, capturedAtDestination))) return true;
        continue;
      }

      if (LineOfSightRules.HasClearAttackPath(attackerRule, OccupiedSquares(attackerRule, (attacker.X, attacker.Y)), square,
        match.Terrain.IsForest, match.Barricades.ContainsKey,
        intermediate => match.Pieces.Any(other =>
          ...
          UnitRules.TryGet(other.Type, out UnitRule otherRule) && OccupiedSquares(otherRule, (other.X, other.Y)).Contains(intermediate)))) return true;
    }
    return false;
  }

  private static bool ServerChessThreatBlocker(Match match, (int x, int y) square,
    NetworkPiece attacker, NetworkPiece king, NetworkPiece? capturedAtDestination) =>
    match.Terrain.IsLake(square) || match.Barricades.ContainsKey(square) || match.Pieces.Any(other =>
      other.Id != attacker.Id && other.Id != king.Id && other.Id != capturedAtDestination?.Id &&
      other.AttachedToId is null && other.Type != nameof(PieceType.Farm) &&
      UnitRules.TryGet(other.Type, out UnitRule otherRule) && OccupiedSquares(otherRule, (other.X, other.Y)).Contains(square));
}
